...
// Provider: 로드된 YAML 맵 데이터를 기반으로 메시지 템플릿을 제공하고 파라미터를 치환하는 컴포넌트
type Provider struct {
	root map[string]any
}

// NewFromYAML: YAML 문자열을 파싱하여 Provider 인스턴스를 생성한다.
...
// NewFromYAMLAtPath: 전체 YAML 컨텐츠 중 특정 경로(dot-notation)에 위치한 하위 객체만을 루트로 사용하는 Provider를 생성한다.
...
// Get: 점(.)으로 구분된 키 경로를 사용하여 메시지 템플릿을 찾고, 제공된 파라미터로 플레이스홀더({key})를 치환하여 반환한다.
