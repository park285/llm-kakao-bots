...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{"valid http", "http://example.com", false, false},
		{"valid https", "https://example.com", false, false},
		{"valid grpc", "grpc://example.com:40528", false, false},
		{"valid grpc no port", "grpc://example.com", false, false},
		{"grpcs not allowed", "grpcs://example.com:40528", false, true},
		{"require grpc blocks http", "http://example.com", true, true},
		{"require grpc allows grpc", "grpc://example.com:40528", true, false},
		{"empty", "", false, true},
		{"invalid url", "://invalid", false, true},
		{"no scheme", "example.com", false, true}, // scheme이 없으면 호스트 파싱이 다름, 하지만 Client 로직상 Scheme 체크함
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := New(Config{BaseURL: tt.baseURL, RequireGRPC: tt.requireGRPC})
