...
import { ScrollText, RefreshCw, AlertTriangle, Shield, Activity, ChevronDown, ChevronRight, Server, Terminal, LayoutList, FileText } from 'lucide-react'
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-2 rounded-xl border border-slate-200 shadow-sm shrink-0">
                <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg w-full sm:w-auto">
                    <button
                        ...
                        className={clsx(
                            "flex items-center gap-2 px-3 py-2 rounded-md text-sm font-bold transition-all flex-1 sm:flex-none justify-center",
                            viewMode === 'audit'
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto px-2 justify-end">
                        ...
                        {logSource === 'combined' && (
                            <button
                                ...
                                title="새로고침"
                            >
                                <RefreshCw size={18} className={systemLogsRefetching ? "animate-spin text-emerald-500" : ""} />
                            </button>
                        )}
                    </div>
                )}
                    ...
                    <>
                        {logSource === 'combined' ? (
                            <div className="flex flex-col h-full">
                                ...
                                <div className="p-2 border-t border-slate-200 bg-white text-[11px] text-slate-400 text-center shrink-0">
                                    통합 로그 • {systemLogsData?.count ?? 0} lines • Updates automatically every 5s
                                </div>
                            </div>
                        ) : (
