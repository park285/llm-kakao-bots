...
// NewCacheService 는 동작을 수행한다.
func NewCacheService(cfg Config, logger *zap.Logger) (*Service, error) {
	...
	logger.Info("Cache store connected",
		...
		zap.Int("db", cfg.DB),
		zap.Int("pool_size", constants.ValkeyConfig.BlockingPoolSize),
	)
...
func (c *Service) Get(ctx context.Context, key string, dest any) error {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache get operation failed", zap.String("key", key), zap.Error(resp.Error()))
		return errors.NewCacheError("get failed", "get", key, resp.Error())
	}

	value, err := resp.ToString()
	if err != nil {
		c.logger.Error("Cache value conversion failed", zap.String("key", key), zap.Error(err))
		return errors.NewCacheError("conversion failed", "get", key, err)
	}

	if dest != nil {
		if err := json.Unmarshal([]byte(value), dest); err != nil {
			c.logger.Error("Cache value unmarshal failed", zap.String("key", key), zap.Error(err))
...
func (c *Service) MGet(ctx context.Context, keys []string) (map[string]string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache mget failed", zap.Int("keys", len(keys)), zap.Error(resp.Error()))
...
func (c *Service) MSet(ctx context.Context, pairs map[string]interface{}, ttl time.Duration) error {
	...
	for key, value := range pairs {
		jsonData, err := json.Marshal(value)
		if err != nil {
			c.logger.Warn("Failed to marshal value for MSet", zap.String("key", key), zap.Error(err))
			continue
		}
	...
	for _, resp := range c.client.DoMulti(ctx, cmds...) {
		if resp.Error() != nil {
			c.logger.Error("MSet command failed", zap.Error(resp.Error()))
...
func (c *Service) DelMany(ctx context.Context, keys []string) (int64, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache delete many failed", zap.Int("count", len(keys)), zap.Error(resp.Error()))
...
func (c *Service) Keys(ctx context.Context, pattern string) ([]string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache keys search failed", zap.String("pattern", pattern), zap.Error(resp.Error()))
...
func (c *Service) SAdd(ctx context.Context, key string, members []string) (int64, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache sadd failed", zap.String("key", key), zap.Error(resp.Error()))
...
func (c *Service) SRem(ctx context.Context, key string, members []string) (int64, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache srem failed", zap.String("key", key), zap.Error(resp.Error()))
...
func (c *Service) SMembers(ctx context.Context, key string) ([]string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache smembers failed", zap.String("key", key), zap.Error(resp.Error()))
...
func (c *Service) SIsMember(ctx context.Context, key, member string) (bool, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache sismember failed", zap.String("key", key), zap.Error(resp.Error()))
...
func (c *Service) HGet(ctx context.Context, key, field string) (string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache hash get failed", zap.String("key", key), zap.String("field", field), zap.Error(resp.Error()))
...
func (c *Service) HGetAll(ctx context.Context, key string) (map[string]string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache hgetall failed", zap.String("key", key), zap.Error(resp.Error()))
...
func (c *Service) Exists(ctx context.Context, key string) (bool, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Cache exists failed", zap.String("key", key), zap.Error(resp.Error()))
...
// InitializeMemberDatabase 는 동작을 수행한다.
...
func (c *Service) GetMemberChannelID(ctx context.Context, memberName string) (string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Failed to get member channel ID", zap.String("member", memberName), zap.Error(resp.Error()))
...
func (c *Service) GetAllMembers(ctx context.Context) (map[string]string, error) {
	...
	if resp.Error() != nil {
		c.logger.Error("Failed to get all members", zap.Error(resp.Error()))
